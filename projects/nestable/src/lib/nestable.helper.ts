export const _traverseChildren = (tree, callback, parent = null, childrensKey = 'Items') => {
    for (let i = 0; i < tree.length; i++) {
        ...
        if (item[childrensKey]) {
            _traverseChildren(item[childrensKey], callback, item, childrensKey);
        }
    }
};
...
export const _findObjectInTree = (array, id, childrensKey = 'Items') => {
    let result = null;
